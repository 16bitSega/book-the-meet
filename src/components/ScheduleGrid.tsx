...
import { format, addWeeks, subWeeks, startOfWeek, addMinutes, isSameDay, addDays, setHours, setMinutes } from "date-fns";
...
export default function ScheduleGrid({ onSlotClick }: ScheduleGridProps) {
  ...
  // Fetch Bookings whenever week or room changes
  useEffect(() => {
  ...
  const getBookingForCell = useCallback(
    (dayDate: Date, startMinutes: number) => {
      ...
      return bookings.find((b) => {
        ...
        return Math.abs(bStart - slotStartTime) < 60000;
      });
    },
    ...
    <div className="w-full bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden">
      ...
      <div className="p-4 border-b border-slate-200 flex flex-col sm:flex-row justify-between items-center gap-4">
        <div className="flex items-center gap-2">
          <button onClick={handlePrevWeek} className="p-2 hover:bg-slate-100 rounded text-slate-600 font-bold">
            ←
          </button>
          <span className="font-semibold text-slate-800 min-w-[140px] text-center text-sm">
            {format(currentWeekStart, "MMM dd, yyyy")}
          </span>
          <button onClick={handleNextWeek} className="p-2 hover:bg-slate-100 rounded text-slate-600 font-bold">
            →
          </button>
          <button
            onClick={handleToday}
            className="ml-2 px-3 py-1 text-sm bg-slate-100 hover:bg-slate-200 rounded text-slate-700 font-medium"
          >
            Today
          </button>
        </div>
        ...
        <div className="flex items-center gap-4">
          <select
            ...
            className="border border-slate-300 rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none bg-white"
          >
            {rooms.map((r) => (
          ...
          <div className="text-xs bg-blue-50 text-blue-700 px-3 py-1.5 rounded border border-blue-100 hidden md:block">
      ...
      <div className="overflow-x-auto">
        <div className="min-w-[900px]">
          ...
          <div className="grid grid-cols-[60px_repeat(7,1fr)] border-b border-slate-200 bg-slate-50">
            <div className="p-3 text-xs font-bold text-slate-400 uppercase tracking-wider">Time</div>
            {weekDays.map((day, i) => {
              const nowKyiv = toZonedTime(new Date(), KYIV_TIMEZONE);
          ...
          <div className="relative">
            ...
            {timeSlots.map((minutes) => (
              <div
                key={minutes}
                className="grid grid-cols-[60px_repeat(7,1fr)] border-b border-slate-100 hover:bg-slate-50/30 transition-colors"
              >
                {/* Time Label */}
                ...
                {weekDays.map((day, dayIdx) => {
                  ...
                  let cellClass = "border-l border-slate-100 p-1 relative group cursor-pointer transition-colors";

                  if (booking) {
                    cellClass = isMyBooking
                      ? "border-l border-slate-100 p-1 relative bg-blue-50/60 cursor-default"
                      : "border-l border-slate-100 p-1 relative bg-slate-100/60 cursor-not-allowed";

                    cellContent = (
                      <div
                        className={`h-full w-full rounded text-xs p-1.5 flex flex-col justify-center shadow-xs ${
                          isMyBooking
                    ...
                    cellContent = (
                      ...
                      >
                        <div className="w-3 h-3 rounded-full bg-blue-500 shadow-xs group-hover:scale-110 transition-transform"></div>
                      </div>
                    );
                  }
